...
- [x] shared-go pin bumped in `hq/go/go.mod`, `go build ./...` and `go test ./...`
...
- 2026-09-03 11:35 — klan.name and klan.groupName widened to VARCHAR(999) as well (commit 007c0bd), keeping the two team read models consistent. Unlike patrulje this is precautionary, not a reproduced overflow: all 230 hq.klan rows already fit, longest groupName 56 characters and longest name 38. Verified on a freshly created klan table that the wider columns and idx_klan_year_status both come up as expected. Existing databases: ALTER TABLE klan MODIFY name VARCHAR(999) NOT NULL DEFAULT "", MODIFY groupName VARCHAR(999) NOT NULL DEFAULT "";
