...
- [ ] Expanded row shows current status, timestamp and acceptor instead of `{{ data }}`
- [ ] Status can be set to any valid value except `finished`
- [ ] `PUT /api/member/:memberId/status` mints a case, records the correction, closes it,
      in one operation
- [ ] Minted case has a headline identifying it as a manual correction and naming the member
- [ ] The correction appears on that case's timeline
- [ ] The minted case does **not** appear among open cases
- [ ] It does appear in the patrol's **Kontakt med nødtelefon** card
- [ ] Sequencing strictness decided and recorded in the log
- [ ] Dialog defers live updates while open
- [ ] `go build ./... && go vet ./...` clean; `npm run build` and `vitest` clean
- [ ] Exercised against the dev stack: correct a member, then find the record from the
      patrol page alone
