...
//
// The vocabulary itself now lives in `composables/memberStatus.ts` (task 183) and is re-exported
// here, exactly as `severity.ts` is above: person search shows a scout's status too, and a results
// page importing the nødtelefon module to learn what `sheltered` means would be the wrong
// dependency. Re-exported rather than moved outright so this module's own call sites are untouched.
export {
  memberStatusPhrase,
  memberStatusBadge,
  memberStatusColour,
