<template>
    <div class="container p-3">
        <h1 class="mb-2 clearfix">
            <span class="h2 mb-0 float-md-left"><span class="badge badge-dark float-md-left mr-3 text-uppercase easy-select">{{ Number(team.teamNumber) || '&times;&times;' }} - {{ team.memberCount }}</span></span>
            <span class="h3 mb-0 float-md-left d-block d-md-inline">
                <small class="text-uppercase text-muted">{{ team.title }}<small class="ml-1"></small></small>
            </span>
        </h1>
        <hr />
        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <dl class="row pb-0 mb-0">
              ...
              <dd class="col-10">{{ team.title }}</dd>

              <dt class="col-2 small text-uppercase grey">Gruppe / Division</dt>
              <dd class="col-10">{{ team.gruppe }}</dd>
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10"><input type="text" class="form-control" id="teamGruppe" :value="team.gruppe"></div>
            ...
            <table class="table table-sm" style="font-size:0.8rem">
              ...
              <tbody>
                <tr v-for="member in team.members" :key="member.memberId" @click="openMember(member)">
                  <th scope="row">1</th>
                  <td>{{ member.title }}</td>
    ...
    <form>
      ...
      <div class="form-group row">
        ...
        <div class="col-sm-10"><select class="form-control form-control-sm" id="modalFunction">
          <optgroup v-for="(group, name) in memberStati" :key="name" :label="name">
            <option v-for="option in group" :key="option.value" :value="option.value" :selected="member.role==option.value">{{ option.text }}</option>
          </optgroup>
        </select></div>
      </div>
    </form>
            ...
            <div class="card">
              ...
              <div class="card-body"  style="font-size:0.8rem">
                <ul class="list-group list-group-flush">
                  <li v-for="mail in team.mails" :key="mail.sendUts" @click="showMail(mail)" role="button" class="px-0 py-1 list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                    {{ mail.subject }}
              ...
              <div class="card-body" style="font-size:0.8rem">
                <ul class="list-group list-group-flush">
                  <li v-if="team.payments" class="px-0 py-1 list-group-item">
                    <div v-for="payment in team.payments" :key="payment.uts" class="d-flex justify-content-between align-items-center">
                    DKK {{ payment.amount }},-
